# Multi-Drone Hand Gesture Control
-------
...
-------
The main goal of this project is to control multiple types of drones using hand gestures without any gloves or additional equipment.
Currently supports DJI Tello and PX4-based drones. Just camera on the drone or your smartphone(soon), laptop and human hand.<br>
...
This project is designed to run on Linux systems. For automated deployment:

```bash
# 1. Check compatibility
./linux_compatibility_check.sh

# 2. Auto-deploy on Linux
./linux_deploy.sh
...
tensorflow == 2.4.1
mediapipe == 0.8.2
pymavlink == 2.4.41  # For PX4 support
```
...
#### For PX4-based drones:
1. Connect your PX4 flight controller via USB or network
2. Ensure MAVLink communication is working (default port: UDP 14550)
3. Configure your PX4 connection string in `config.txt` (default: `udp:127.0.0.1:14550`)
...
## Usage
The most interesting part is demo. There are 2 types of control: keyboard and gesture. You can change between control types during the flight. The system will automatically detect and connect to available drones (Tello or PX4). Below is a complete description of both types.

Run the following command to start the drone control :

```sh
python3 main.py
```

**Note for PX4 users**: PX4 drones require external video cameras. Make sure your camera is connected and accessible.
...
* `a` -> Move left
* `d` -> Move right
* `e` -> Rotate clockwise (PX4: right turn)
* `q` -> Rotate counter-clockwise (PX4: left turn)
* `r` -> Move up
* `f` -> Move down
* `Esc` -> End program and land the drone

**Note for PX4**: PX4 uses RC control mode for keyboard input, providing smooth and continuous control. 
...
│   │  px4_gesture_controller.py
│   │  px4_keyboard_controller.py
...
- [x] PX4 drone support (MAVLink)
- [ ] DJI drone support (SDK integration)
...
* [PX4 MAVLink Python interface](https://github.com/ArduPilot/pymavlink)
